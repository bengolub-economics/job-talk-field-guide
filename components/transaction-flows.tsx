...
export function TransactionFlows({stage = 0}: {stage?: number}) {
  ...
  const box = (x: number, y: number, label: string, width = 170) => <g>
    <rect x={x} y={y} width={width} height={62} rx={3} fill="#edf4f1" stroke={teal}/>
  ...
  return <svg className="diagram" viewBox="0 0 800 365" role="img" aria-label={descriptions[Math.min(stage, 2)]} style={{fontFamily:'Arial,sans-serif', fill:ink}}>
    ...
    {box(600, 145, 'Firms')}
    <path d="M200 176 H588" fill="none" stroke={teal} strokeWidth={3}/>
    <path d="M600 176 l-12 -6 v12 Z" fill={teal}/>
    <text x={400} y={154} textAnchor="middle" fontSize={24} fill={teal}>Consumption · 522.91</text>
    {stage === 0 && <>
      <text x={115} y={240} textAnchor="middle" fontSize={21} fill={gray}>Payer</text>
      <text x={685} y={240} textAnchor="middle" fontSize={21} fill={gray}>Recipient</text>
    </>}
    {stage >= 1 && <>
      <path d="M640 207 V270 H160 V219" fill="none" stroke={teal} strokeWidth={3}/>
      <path d="M160 207 l-6 12 h12 Z" fill={teal}/>
      <text x={400} y={257} textAnchor="middle" fontSize={24} fill={teal}>Wages · 322.26</text>
      <path d="M710 207 V340 H90 V219" fill="none" stroke={teal} strokeWidth={3}/>
      <path d="M90 207 l-6 12 h12 Z" fill={teal}/>
      <text x={400} y={327} textAnchor="middle" fontSize={24} fill={teal}>Corporate profits · 383.80</text>
    </>}
    {stage >= 2 && <>
