...
export function SlideComparison({before,after,sourceUrl,outline=false}:{before:ViewerPanel[];after:ViewerPanel[];sourceUrl?:string;outline?:boolean}){
 ...
 const chooseAfter=(step:number)=>{
  setAfterStep(step);
  const id=after[step].revises?.[0];
  if(id)chooseBefore(id);
 };
 return <div className={`pair case-comparison ${outline?'outline-comparison':''}`}>
  ...
  <PanelViewer panels={after} side="after" label={outline?'Better outline':'Better'} sourceUrl={sourceUrl} outline={outline} step={afterStep} onStepChange={chooseAfter} beforePanels={before} onChooseBefore={chooseBefore}/>
 </div>;
}
