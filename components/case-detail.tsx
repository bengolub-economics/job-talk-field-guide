...
export function CaseDetail({item:c}:{item:Case}){
 ...
 return <main id="main" className="wrap case-page"><a className="back-link" href={sitePath('/')}>← All cases</a>
  ...
  {c.source&&<section className="case-source-compact"><p><a target="_blank" rel="noreferrer" href={`${c.source.url}#page=${c.source.pages.match(/\d+/)?.[0]}`}>{c.source.title} ↗</a><br/>{c.source.authors} · {c.source.date} · PDF {c.source.pages}</p></section>}
