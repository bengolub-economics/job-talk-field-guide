...
export function CaseDetail({item:c}:{item:Case}){
 ...
 return <main id="main" className="wrap case-page"><a className="back-link" href="/">← All twelve cases</a>
  ...
  <section className="case-point"><p><MathText>{c.point||c.diagnosis}</MathText></p><a href={`/essay.html#${principleAnchors[c.principle]}`}>The essay’s advice ↗</a></section>
  ...
  <nav className="next-case" aria-label="Next case"><span>NEXT CASE</span><a href={`/case/${next.id}`}>{next.number} / {next.title} <b>→</b></a></nav>
 </main>;
}
