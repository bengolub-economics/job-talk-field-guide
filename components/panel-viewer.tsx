...
export function PanelViewer({panels,side,label,caption,sourceUrl,outline=false,step:controlledStep,onStepChange,beforePanels,onChooseBefore}:{panels:ViewerPanel[];side:'before'|'after';label:string;caption?:string;sourceUrl?:string;outline?:boolean;step?:number;onStepChange?:(step:number)=>void;beforePanels?:ViewerPanel[];onChooseBefore?:(id:string)=>void}){
 ...
 return <section className={`panel-viewer ${side}`}>
  <div className="viewer-heading"><h2>{label}</h2><Dialog><DialogTrigger className="enlarge-button">Enlarge ↗</DialogTrigger><DialogContent className="slide-dialog"><DialogTitle>{label}</DialogTitle><DialogDescription className="sr-only">{p.title}</DialogDescription>{selector}<div className="zoom-tools"><button onClick={()=>setLarge(!large)}>{large?'Fit to screen':'Larger text'}</button></div><div className="zoom-window" tabIndex={0} role="region" aria-label="Enlarged slide"><div className={large?'zoom-canvas native-size':'zoom-canvas'}><Slide panel={p} variant={side}/></div></div>{controls}</DialogContent></Dialog></div>
